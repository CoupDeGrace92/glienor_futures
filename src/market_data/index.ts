...
import { WGMarketClient, WGDumpData } from "./market_client.js";
...
const popClient = new WGMarketClient("glienor futures data fetcher");
const data= await popClient.getAllItems();

let timestampObj = await GetLastUpdateTimestamp();
let fetchedTimestamp: Date;
...
if ("success" in data) {
    ...
    if (isNaN(fetchedTimestamp.getTime())) {
        ...
        } else (console.log("/exchange timestamp: ", fetchedTimestamp));
    } else (console.log("%JAGEX_TIMESTAMP%: ", fetchedTimestamp));
...
async function fastChunkedDailyInsert(rawDump: WGDumpData, timestamp: Date) {
